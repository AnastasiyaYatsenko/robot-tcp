...
class Ceil:
    ...
    def add_robot(self, ip):
        ...
        x1_coord, y1_coord = ceil_to_coordinates(x1, y1)
        x2_coord, y2_coord = ceil_to_coordinates(x2, y2)
        x3_coord, y3_coord = ceil_to_coordinates(x3, y3)
        print(f"hand1: ({x1_coord}, {y1_coord})\nhand2: ({x2_coord}, {y2_coord})\nhand3: ({x3_coord}, {y3_coord})")
        r.set_coordinates(x1_coord, y1_coord,
                          x2_coord, y2_coord,
                          x3_coord, y3_coord)
    ...
    def is_point_free(self, x, y, robot_num):
        ...
        x_ceil, y_ceil = ceil_to_coordinates(x, y)
    ...
    def move_hand(self, robot_num, hand_num, x, y):
        ...
        print(f"HAND {hand_num} to CEIL x: {ceil_x}, y: {ceil_y}")
    ...
    def is_move_possible_two_holds(self, robot_num, xo_s, yo_s, xo_t, yo_t, hand_coords, hand_c):
        ...
        for n in range(N + 1):
            ...
            if 0 < n < N:
                ...
                yo_n = yo_t
            # print("---------")
            # print(f"CHECK s X: {xo_s} Y: {yo_s}")
            # print(f"CHECK n X: {xo_n} Y: {yo_n}")
            # print(f"CHECK t X: {xo_t} Y: {yo_t}")
            # print("---------")
    ...
    def find_pos_by_shifts(self, robot_num, xo_s, yo_s, hand_coords, start_shifts, hand_c = -1):
        ...
        min1, max1 = (int(max(start_shifts[0] - 2*size["innerRadLimit"], size["innerRadLimit"])),
                      int(min(start_shifts[0] + 2*size["innerRadLimit"] + 1, size["outerRadLimit"] + 1)))
        min2, max2 = (int(max(start_shifts[1] - 2*size["innerRadLimit"], size["innerRadLimit"])),
                      int(min(start_shifts[1] + 2*size["innerRadLimit"] + 1, size["outerRadLimit"] + 1)))
        min3, max3 = (int(max(start_shifts[2] - 2*size["innerRadLimit"], size["innerRadLimit"])),
                      int(min(start_shifts[2] + 2*size["innerRadLimit"] + 1, size["outerRadLimit"] + 1)))
    ...
    def move_vector(self, robot_num, xo_s, yo_s, xo_t, yo_t, hand_coords, move_hand = -1):
        ...
        # print(f"Os: ({xo_s}, {yo_s}) Ot: ({xo_t}, {yo_t}), self.N = {self.N}")
    ...
    '''
    def get_neighbours_line(self, hand_coords):
        neighbours = []
   ...
   '''
    ...
    def build_path_2(self, robot_num, xo_s, yo_s, xo_t, yo_t):
        ...
        while not move_finished and counter <= 2000:
            ...
            if pos_conditions:
                ...
                best_center = (-1, -1)
                # hand_found = False
                print(f"Neighbours amount: {len(neighbours)}")
                for point in sorted_neighbours:
                    if point_found:
                        print(f"in the start point_found={point_found} and move_finished={move_finished}")
                        break
                    print(f"Point: {point}")
                    print(f"Hand coords: {hand_coords}")
                    if not point in opt_points:
                    ...
                    for j in range(3):
                        ...
                        temp_coords[j] = point
                        print(f"IN LOOP Temp coords: {temp_coords}")
                        new_x, new_y = self.check_point(robot_num, temp_coords, j, center_x, center_y, point,
                                                        opt_points)
                        print(f"Hand {j}: ({new_x}, {new_y})")
                        # print(f"Temp coords: {temp_hand_coords}")
                        # print(f"Loop optimal: {opt_points}")
                        if new_x != -1 and new_y != -1:
                            ...
                            if temp_coords in path:
                                ...
                                print(f"Adding the position: {temp_coords}")
                                # best_pos = temp_coords[:]
                                # best_center = (new_x, new_y)
                                point_found = True
                                if best_center[0] == -1:
                                    best_pos = temp_coords[:]
                                    best_center = (new_x, new_y)
                                else:
                                    d1 = dist(best_center[0], best_center[1], xo_t, yo_t)
                                    d2 = dist(new_x, new_y, xo_t, yo_t)

                                    d1_ = get_dist_point_line(best_center[0], best_center[1], a, b, c)
                                    d2_ = get_dist_point_line(new_x, new_y, a, b, c)
                                    print(f"Dist1: {d1} Dist 2: {d2}")
                                    if d1 > d2:
                                        best_pos = temp_coords[:]
                                        best_center = (new_x, new_y)
                        ...
                        #     break
                        if move_finished:
                            print(f"in the middle point_found={point_found} and move_finished={move_finished}")
                            break
                if not point_found:
                    print("Didn't find the point!!!")
                    move_finished = True
                    ...
                    # TODO check if we are trying to move tha same hand
                    print("point found, try to append")
                    move_hand_prev = -1
                    move_hand_now = -1
                    if len(path) >= 2:
                        for j in range(3):
                            if (path[len(path)-2][j][0] != path[len(path)-1][j][0] or
                                ...
                                if move_hand_prev != -1:
                                    ...
                                    move_hand_prev = j
                        for j in range(3):
                            if (path[len(path)-1][j][0] != best_pos[j][0] or
                                    path[len(path)-1][j][1] != best_pos[j][1]):
                                if move_hand_now != -1:
                                    print("Need to move two hands at the same time, impossible!")
                                    return -1
                                else:
                    ...
                    print(f"appending the point, old center: ({center_x}, {center_y}), coords: {hand_coords}")
                    ...
                    hand_coords = best_pos[:]
                    print(f"new center: ({center_x}, {center_y}), coords: {hand_coords}")
                    # break
                # if point_found or move_finished:
                #     print(f"in the end point_found={point_found} and move_finished={move_finished}")
                    # break
        ...
        # first we determine optimal path by holes, then robot moves to a specified holes
    def build_path_3(self, robot_num, xo_s, yo_s, xo_t, yo_t):
        self.robots[robot_num].isMovingPath = True
        self.robots[robot_num].curr_index = -1
        a, b, c = get_line_equation(xo_s, yo_s, xo_t, yo_t)
    ...
    def build_path_hex(self, robot_num, xo_s, yo_s, xo_t, yo_t):
        # is_ot_possible = self.check_if_ot_possible(xo_t, yo_t)
    ...
    def turn_clock(self, robot_num, clockwise = True):
        ...
        for i in range(len(possible_first_move)):
            ...
            # is_start_inside_triangle = isInside(possible_first_move[i][0][0], possible_first_move[i][0][1],
            #                                     possible_first_move[i][1][0], possible_first_move[i][1][1],
            #                                     possible_first_move[i][2][0], possible_first_move[i][2][1],
            #                                     hand_coords[3][0], hand_coords[3][1])
            # is_curr_inside_triangle = isInside(possible_first_move[i][0][0], possible_first_move[i][0][1],
            #                                    possible_first_move[i][1][0], possible_first_move[i][1][1],
            #                                    possible_first_move[i][2][0], possible_first_move[i][2][1],
            #                                    new_center_x, new_center_y)
            # print(f"Triangle: {is_start_inside_triangle} vs {is_curr_inside_triangle}")
    ...
    def path_manual(self, robot_num, direction):
        ...
        if direction == 0:
            ...
            left_x_ceil = mid_x_ceil - size["d2"] / 2
            ...
            new_left = sorted_points[1]
            mid_x_ceil, mid_y_ceil = ceil_to_coordinates(new_middle[0], new_middle[1])
            right_x_ceil = mid_x_ceil + size["d2"] / 2
